...
func TestConverse_CallsToolThenAnswers(t *testing.T) {
	...
	out, err := e.Converse(context.Background(), "SYS", "what's running?", "",
		[]feature.Tool{brainSafe("scan", "2 live agents", &scanned)}, feature.Gate{})
...
func TestConverse_ScrubsWholeRenderedPrompt(t *testing.T) {
	...
	_, err := e.Converse(context.Background(), "SYS", "peek", "",
		[]feature.Tool{brainSafe("scan", "env: TOKEN=sk-SECRET-TOKEN", &scanned)}, feature.Gate{})
	require.NoError(t, err)
...
func TestConverse_SystemPreambleReachesModel(t *testing.T) {
	...
	_, err := e.Converse(context.Background(), "MANDATE: call scan for fleet questions", "hi", "", nil, feature.Gate{})
	require.NoError(t, err)
...
func TestConverse_ScrubsAnswerBeforeReturn(t *testing.T) {
	...
	out, err := e.Converse(context.Background(), "SYS", "what's the token?", "", nil, feature.Gate{})
	require.NoError(t, err)
...
func TestConverse_InjectsSessionHistoryTool(t *testing.T) {
	...
	out, err := e.Converse(ctx, "SYS", "what did we discuss earlier?", "S1", nil, feature.Gate{})
	require.NoError(t, err)
...
func TestConverse_NoHistoryToolWithoutSession(t *testing.T) {
	...
	_, err := e.Converse(context.Background(), "SYS", "hi", "", nil, feature.Gate{})
	require.NoError(t, err)
...
func TestConverse_ProseReplyPassthrough(t *testing.T) {
	...
	out, err := e.Converse(context.Background(), "SYS", "status?", "", nil, feature.Gate{})
	require.NoError(t, err)
